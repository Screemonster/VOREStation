//
// RUST Fuel Injector
//	This machine actually injects fuel into the EM field
//
...
/obj/machinery/power/rust_fuel_injector
	...
	circuit = /obj/item/weapon/circuitboard/rust_fuel_injector
...
/obj/machinery/power/rust_fuel_injector/interact(mob/user)
	if(stat & BROKEN)
		user.unset_machine()
		user << browse(null, "window=rust_fuel_injector")
		return
	...
	user << browse(dat, "window=rust_fuel_injector;size=500x300")
	onclose(user, "rust_fuel_injector")
	...
	if(cur_assembly)
		...
		for(var/reagent in cur_assembly.rod_quantities)
			...
			if(cur_assembly.rod_quantities[reagent] > 0)
				...
				var/obj/effect/accelerated_particle/rust/A = new/obj/effect/accelerated_particle/rust(get_turf(src), dir)
...
//
// An extention of the normal accelerated particle that will interact with our stuff!
//

/obj/effect/accelerated_particle/rust/Bump(atom/A)
	if(A && istype(A, /obj/effect/rust_particle_catcher) )
		...
		if(particle_type && particle_type != "neutron")
			if(collided_catcher.AddParticles(particle_type, 1 + additional_particles))
				collided_catcher.parent.AddEnergy(energy)
				qdel(src)
	else
		return ..()
