...
/obj/item/frame/rust_fuel_assembly_port
	name = "fuel assembly port frame"
	desc = "Used for building fuel assembly ports.  Build on the other side of a wall from a fuel injector."
	icon = 'icons/rust.dmi'
	icon_state = "port2"
	//build_machine_type = /obj/item/rust_fuel_assembly_port_construct
	// TODO - For now just build the damn thing. we'll do fancy construction later
...
/obj/item/rust_fuel_assembly_port_construct
	name = "fuel assembly port frame"
	desc = "A fuel assembly port under construction."
	icon = 'icons/rust.dmi'
	icon_state = "port2"
	anchored = 1
	w_class = 4
...
/obj/item/rust_fuel_assembly_port_construct/examine(mob/user)
	if(!..(user, 2))
		return

	switch(src.has_electronics)
		if(0)
			user << "It's an empty frame."
			return
		if(1)
			user << "It has a circuit board."
			return
		if(2)
			user << "It's wired."
			return

/obj/item/rust_fuel_assembly_port_construct/attackby(obj/item/weapon/W as obj, mob/user as mob)
